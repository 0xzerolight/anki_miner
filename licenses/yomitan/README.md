...
The deinflection engine, its Japanese rule table, **and their test corpus** are
Python ports of code from [Yomitan](https://github.com/yomidevs/yomitan),
Copyright (C) Yomitan Authors, licensed under the **GNU General Public License,
version 3 or (at your option) any later version** — the full text is in
[`COPYING.GPLv3`](COPYING.GPLv3). Anki Miner is itself GPL-3.0-or-later, so the
combined work carries the same license.
