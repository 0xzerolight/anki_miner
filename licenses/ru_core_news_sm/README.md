...
Anki Miner uses spaCy's `ru_core_news_sm` 3.8.0 as the Russian tagger. The model is licensed MIT
(copyright ExplosionAI GmbH); the licence text is in `LICENSE` and the source pointer in
`SOURCES.txt`. Its training data is the Nerus corpus (MIT, Alexander Kukushkin). Its lemmatiser
reads the pymorphy3 Russian dictionaries, whose notice is `licenses/pymorphy3_dicts_ru/`.
...
Explosion, verified against the SHA-256 digest pinned in `anki_miner/languages/ru/pack.py` and
extracted to `~/.anki_miner/language_packs/ru/`, where the wheel's own `LICENSE` and
