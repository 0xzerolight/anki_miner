...
### Changed
- **Duplicates can backfill existing cards instead of only being skipped (opt-in).** A new `duplicate_behavior` setting (`skip` by default, unchanged) can be set to `update`: when a mined word already has a card, its EMPTY mapped fields are filled from this run (word audio, pitch, frequency, etc. you mapped later) while any field you have edited is left untouched — following Yomitan's coalesce merge. The existing card is located by a first-field query scoped to your Duplicate Scope; a card that can't be found stays skipped. Updated cards are not new cards, so Undo never deletes or blanks them. The run summary reports "Updated N existing card(s)".
...
### Fixed
- **Mined cards were unstyled by default after the v2.7.7 update.** The dictionary CSS that used to be baked into each card moved into a single managed note-type stylesheet in the rework — but that stylesheet was off by default, so newly mined cards (and Jitendex/Lapis glossaries) rendered with no styling: part-of-speech tags ran together, sense structure and example-sentence styling were gone. Card styling now defaults on; enabling it restyles existing cards the moment it syncs, no re-mining needed.
