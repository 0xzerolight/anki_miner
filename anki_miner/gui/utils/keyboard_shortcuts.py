...
3. **One source for what is bound and what is advertised.** Every remappable
   key is a row of ``key_bindings.KEY_ACTIONS``, and the About card's table is
   generated from the live bindings (``key_bindings.about_rows``), so a
   rebinding cannot leave the printed list behind.
