...
class ExportDialog(QDialog):
    ...
    def _setup_ui(self) -> None:
        ...
        # Header
        header = SectionHeader(self.tr("Export Words"))
        ...
        format_label = QLabel(self.tr("Export Format"))
        format_label.setFont(self._make_font(13, QFont.Weight.Bold))
        format_layout.addWidget(format_label)

        self._format_group = QButtonGroup(self)
        self._radio_csv = QRadioButton(self.tr("CSV (.csv)"))
        self._radio_tsv = QRadioButton(self.tr("TSV (.tsv)"))
        self._radio_vocab = QRadioButton(self.tr("Vocabulary List (.txt)"))
        ...
        vocab_fmt_label = QLabel(self.tr("List format:"))
        vocab_opts_layout.addWidget(vocab_fmt_label)

        self._vocab_format_combo = QComboBox()
        self._vocab_format_combo.addItems([self.tr("Plain (one word per line)"), "Takoboto", "jpdb"])
        ...
        preview_text = tr_format(self.tr("%1 words"), len(self._words))
        if with_def:
            preview_text += tr_format(self.tr(", %1 with definitions"), with_def)
        if with_media:
            preview_text += tr_format(self.tr(", %1 with media"), with_media)
        ...
        self._path_input.setPlaceholderText(self.tr("Select output file..."))
        self._path_input.setReadOnly(True)
        path_layout.addWidget(self._path_input)

        browse_btn = ModernButton(self.tr("Browse..."), variant="secondary")
        ...
        cancel_btn = ModernButton(self.tr("Cancel"), variant="ghost")
