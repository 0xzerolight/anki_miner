...
class YouTubeQueueWorker(CancellableWorker):
    ...
    def _mine_one(self, idx: int, item: YouTubeQueueItem, workspace: Path) -> object:
        ...
        ``video_id`` and ``resolved_sub_mode``). The assertions narrow
        Optional types from the queue model for mypy.
        """
        assert item.video_id is not None, "READY items must have video_id"
        assert item.resolved_sub_mode is not None, "READY items must have resolved_sub_mode"
