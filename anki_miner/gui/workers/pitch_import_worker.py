"""QThread worker that wraps the Yomitan pitch-accent importer with progress + cancel.

Runs ``import_yomitan_pitch_zip`` off the GUI thread and surfaces progress,
completion, and failure as Qt signals. Direct structural mirror of
:class:`FrequencyImportWorker` so the two import flows stay symmetric for
future maintainers.
"""
...
class PitchImportWorker(CancellableWorker):
    """Imports a Yomitan-format pitch-accent zip into a CSV in the background.
    ...
    Signals:
        ...
        import_finished(object): emitted once on success with the
            :class:`YomitanPitchImportResult`. Typed as ``object`` to dodge
