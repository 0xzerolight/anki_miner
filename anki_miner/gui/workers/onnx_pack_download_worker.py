"""Worker that downloads and installs the onnxruntime pack (Silero VAD).

The pack is a single wheel whose byte total is known up front (the
``Content-Length`` header), so this worker converts the underlying download
progress into a percentage status line — like ``CudaPackDownloadWorker``.
...
cancel path) stays intact for lifecycle release.
"""
...
class OnnxPackDownloadWorker(CancellableWorker):
    """Download and install the onnxruntime pack in a background thread.

    Args:
        onnx_pack_root: Managed directory for the pack; typically
            ``config.onnx_pack_root``.
    ...
    def __init__(self, onnx_pack_root: Path, parent=None) -> None:
    ...
    def run(self) -> None:
        ...
        Emits ``status`` before starting, then calls ``install_onnx_pack`` with a
        progress adapter that surfaces a percentage. Any exception is caught and
        forwarded as ``result_ready(False, error_text)``.
        """
        if self.check_cancelled():
            return

        self.status.emit(self.tr("Downloading silence-removal library…"))

        try:
            install_onnx_pack(
                self._onnx_pack_root,
                progress=self._on_progress,
                cancel_event=self._cancel_event,
            )
        except Exception as exc:  # noqa: BLE001 — surface every failure to GUI
            logger.exception("onnxruntime pack install failed")
            if not self.check_cancelled():
                self.result_ready.emit(False, str(exc))
            return

        if not self.check_cancelled():
            self.result_ready.emit(True, self.tr("Silence-removal library installed successfully."))
