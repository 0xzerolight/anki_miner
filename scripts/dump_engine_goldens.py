...
# The v2 companion imports this module by its stable name.  Preserve one
# exception type when the exporter itself is launched as ``__main__``.
if __name__ == "__main__":
    sys.modules["dump_engine_goldens"] = sys.modules[__name__]
...
def _runtime_provenance(
    engine_root: Path,
    distributions: Sequence[tuple[str, tuple[str, ...]]] = RUNTIME_DISTRIBUTIONS,
) -> dict[str, Any]:
    dependencies: dict[str, dict[str, str]] = {}
    for distribution_name, import_names in distributions:
        try:
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        choices=(1, 2),
        default=1,
        help="golden contract version (v1 remains available for historical re-derivation)",
    )
    parser.add_argument(
        "--v2-input",
        type=Path,
        default=repository_root / "tests" / "fixtures" / "goldens" / "engine-v2-input.json",
        help="reviewed v2 contract input",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.schema_version == 1:
            ...
            from engine_golden_contract_v2 import build_goldens_v2

            result = build_goldens_v2(
                engine_root=args.engine_root,
                tokenizer_corpus_path=args.corpus,
                contract_input_path=args.v2_input,
                dicdir=args.dicdir,
                assets=assets,
            )
