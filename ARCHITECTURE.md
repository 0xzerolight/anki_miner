...
- **AnkiService**: AnkiConnect HTTP API wrapper (localhost:8765). Key operations: `verify_card_target` (pre-flight — checks the note type, validates the field mapping, and asserts the target deck already exists; it creates **nothing**, `ensure_deck` is Deck-Filter-only), `get_existing_vocabulary`, `create_cards_batch` (batch size 100), `delete_notes`. Stores `last_created_note_ids` for undo and `anki_write_state` as the run's write provenance. Three collaborators are split out so they are testable without HTTP mocks: `services/_ankiconnect.py` (the shared `post_action` transport, a deliberately stable patch target), `services/anki_note_builder.py` (CardPayload → note dict), and `services/anki_media_store.py` (chunked `storeMediaFile` uploads with per-file fallback).
...
`AnkiMinerConfig` (`config/config.py`) is a frozen (immutable) dataclass of 128 fields. Grouped by area (not every field is listed):
...
Mining screens share a base chain in `gui/widgets/`. `MiningTabBase(RunOptionsMixin, TaskPublisherMixin, ScreenIssueHost, QWidget)` is the root — `SingleEpisodeTab` and `BatchProcessingTab` subclass it directly. The queue-driven screens go one level deeper through `_QueueMiningTabBase`, which splits into `_ListQueueMiningTabBase` (the `QListWidget`-queue screens: `YouTubeTab`, `AudiobookTab`) and `_ReadingMiningTabBase` (the four Reading sub-tabs). `TaskPublisherMixin` is what makes every one of them publish into the task registry.
...
- **Mining**: `EpisodeWorkerThread` (one pair) and `BatchQueueWorkerThread` (many pairs, with the season two-pass path).
