...
- `setup_wizard/` (`SetupWizard`, `pages.py`): the guided first-run `QWizard` — theme, mining language, AnkiConnect reachability, target deck, note type + field mapping, recommended resources. The mining-language step is registered only for the first-run offer (`offer_mining_language`) and commits through `switch_language`, so the deck, note type and catalogue every later step derives are the chosen language's own. **Detect-and-guide only**: it never creates decks or note types via AnkiConnect. The user performs every Anki-side action while the wizard inspects, explains, links, and re-checks live rather than trusting a result cached when the page was built. Re-runnable from Tools → Setup Wizard.
