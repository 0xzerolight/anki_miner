"""Italian known-word ingestion through the real AnkiService scan (S15 contamination, S3 fold).

The Latin gate cannot tell Italian from Spanish: a Spanish deck's words land in
the Italian known-words set unless the deck is excluded. The fake AnkiConnect
...
def _config(fake_anki, home, **overrides) -> AnkiMinerConfig:
    base = switch_language(AnkiMinerConfig(), "it")
...
def test_a_latin_gate_ingests_a_spanish_deck_and_folds_italian_fronts(fake_anki, isolated_home):
    config = _config(fake_anki, isolated_home)
    service = AnkiService(config)
    _seed(service, "Italiano", "l'acqua", "Il Gatto")
    _seed(service, "Español", "hola")
    _seed(service, "Japanese", "日本語")

    vocab = service.get_existing_vocabulary()

    assert {"acqua", "gatto"} <= vocab  # S3: the elided-article and article folds at the Anki boundary
    assert "hola" in vocab  # the contamination the first-switch checklist exists for
    assert "日本語" not in vocab
    assert resolve_known_words_db_path(config).name == "known_words.it.db"
