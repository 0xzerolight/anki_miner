...
def test_process_drives_real_tab_fake_anki(tmp_path: Path, qtbot, fake_anki) -> None:
    ...
    Stage 5 of 5, the deck read-back through the real ``AnkiGateway`` matches, a
    ...
    try:
        ...
        assert "Stage 1 of 5" in log, f"'Stage 1 of 5' not found in log: {log[:500]}"
        assert "Stage 2 of 5" in log, f"'Stage 2 of 5' not found in log: {log[:500]}"
        assert "Stage 5 of 5" in log, f"'Stage 5 of 5' not found in log: {log[:500]}"
    finally:
        driver.teardown()
