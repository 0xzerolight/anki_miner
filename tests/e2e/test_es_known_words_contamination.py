"""Spanish known-word ingestion through the real AnkiService scan (S15 contamination, S3 fold).

The Latin gate cannot tell Spanish from English: an English deck's words land in
the Spanish known-words set unless the deck is excluded. The fake AnkiConnect
...
def _config(fake_anki, home, **overrides) -> AnkiMinerConfig:
    base = switch_language(AnkiMinerConfig(), "es")
...
def test_a_latin_gate_ingests_an_english_deck_and_folds_spanish_fronts(fake_anki, isolated_home):
    config = _config(fake_anki, isolated_home)
    service = AnkiService(config)
    _seed(service, "Español", "el perro", "La Casa", "comer")
    _seed(service, "English", "house")
    _seed(service, "Japanese", "日本語")

    vocab = service.get_existing_vocabulary()

    assert {"perro", "casa", "comer"} <= vocab  # S3: the leading-article fold at the Anki boundary
    assert "house" in vocab  # the contamination the first-switch checklist exists for
    assert "日本語" not in vocab
    assert resolve_known_words_db_path(config).name == "known_words.es.db"
