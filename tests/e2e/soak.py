...
_LOG_MARKER_PROCESS_ONLY = "Stage 5 of 5"
...
def _check_gui_state(
    ...
    *,
    ...
    ``cards_created`` gates the card-creation-phase assertions (``Stage 5 of 5`` and
    ...
    — a run that created cards yet has no Stage 5 of 5 or stuck progress is still
    a genuine FAIL.
    """
    checks: dict = {}
    ...
    # 3. Common phase markers appear IN ORDER in the log (Stage 1 of 5 then Stage 2 of 5).
    ...
    # 4. Phase-5 marker: Stage 5 of 5, gated on cards_created > 0: when all words
    # are already known the pipeline returns early (before phase 5), so Stage 5 of 5
    ...
    # but no Stage 5 of 5) still fails.
