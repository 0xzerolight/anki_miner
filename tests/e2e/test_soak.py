...
def test_inprocess_fake_soak_gui_checks_populated_and_pass(
    ...
    through Stage 5 of 5 present, progress advanced).  A failing check would also
    ...
    for s in soak.sessions:
        ...
        assert "log_contains:Stage 1 of 5" in s.gui_checks
        assert "log_contains:Stage 2 of 5" in s.gui_checks
        ...
        assert "log_contains:Stage 5 of 5" in s.gui_checks
        assert s.gui_checks["log_contains:Stage 5 of 5"]["ok"] is True
...
def test_check_gui_state_process_cards_created_zero_no_step5_passes() -> None:
    """Process run, cards_created==0, log has Stage 1 of 5+2/5 but NOT Stage 5 of 5 → PASS.

    Models the faithful soak early-return path: all words already known, pipeline
    returns before phase 5, progress never advances.  Both log_contains:Stage 5 of 5
    ...
    # Stage 5 of 5 absent from log but ok=True because cards_created==0.
    step5 = checks.get("log_contains:Stage 5 of 5")
    assert step5 is not None, "log_contains:Stage 5 of 5 must be recorded"
    assert step5["actual"] is False, "Stage 5 of 5 should not be in log"
    ...
    # All other checks pass (buttons idle, log non-empty, Stage 1 of 5 + 2/5 present).
...
def test_check_gui_state_process_cards_created_positive_missing_step5_fails() -> None:
    """Process run, cards_created>0, log MISSING Stage 5 of 5 → FAIL.

    Regression guard: a genuine bug (cards created but phase-5 marker absent)
    must still be caught.  ok must be False for log_contains:Stage 5 of 5.
    """
    log = "Stage 1 of 5\nStage 2 of 5\n"  # Stage 5 of 5 deliberately absent
    ...
    assert step5["ok"] is False, "ok must be False when cards_created>0 and Stage 5 of 5 absent"
...
def test_check_gui_state_process_cards_created_positive_healthy_passes() -> None:
    """Process run, cards_created>0, Stage 5 of 5 present, progress advanced → PASS.
    ...
    step5 = checks["log_contains:Stage 5 of 5"]
