...
def test_the_filtering_panel_matches_its_navigator_entry(qtbot):
    """The panel title and the navigator label must be the same words (T9 renamed both)."""
    from anki_miner.gui.widgets.panels import FilteringSettingsPanel

    panel = FilteringSettingsPanel()
    qtbot.addWidget(panel)
    assert panel._title_label.text() == "Word Filters"


def test_the_sentences_panel_matches_its_navigator_entry(qtbot):
    """The panel title and the navigator label must be the same words."""
    from anki_miner.gui.widgets.panels import SentencesSettingsPanel
