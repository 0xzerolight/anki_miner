...
def test_measure_word_parses_cc_cedict_cl_gloss(gloss, expected):
    out = ZhMeasureWordHook().render(_word(definition_html=gloss))
...
def test_traditional_hook_emits_only_a_real_variant(monkeypatch):
    ...
    assert ZhTraditionalHook().render(_word("银行")) == {"expression_traditional": "銀行"}
    assert ZhTraditionalHook().render(_word("你好")) == {}
...
def test_tone_colour_spans_are_self_contained_and_escaped(monkeypatch):
    ...
    html_out = ZhToneColorHook().render(_word("银行"))["expression_pinyin"]
...
def test_hooks_return_empty_dicts_rather_than_raising(monkeypatch):
    ...
    assert ZhToneColorHook().render(_word("")) == {}
    assert ZhMeasureWordHook().render(_word()) == {}
