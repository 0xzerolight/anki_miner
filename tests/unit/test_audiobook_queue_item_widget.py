...
def test_processing_glyph() -> None:
...
def test_processing_keeps_subtitle_detail() -> None:
    item = _make_item(status=AudiobookItemStatus.PROCESSING)
    widget = AudiobookQueueItemWidget(item)
    assert widget.detail_label.full_text == "novel_volume_1.srt"
...
def test_completed_glyph() -> None:
    item = _make_item(status=AudiobookItemStatus.COMPLETED)
    widget = AudiobookQueueItemWidget(item)
    assert widget.status_label.text() == "✓"


def test_completed_detail_shows_card_count() -> None:
    item = _make_item()
    widget = AudiobookQueueItemWidget(item)
...
def test_completed_remove_enabled() -> None:
    item = _make_item(status=AudiobookItemStatus.COMPLETED)
    widget = AudiobookQueueItemWidget(item)
    assert widget.remove_button.isEnabled()
...
def test_error_detail_shows_error_message() -> None:
    item = _make_item()
    widget = AudiobookQueueItemWidget(item)
...
def test_error_without_message_shows_empty_detail() -> None:
    item = _make_item(status=AudiobookItemStatus.ERROR)
    widget = AudiobookQueueItemWidget(item)
    assert widget.detail_label.full_text == ""


def test_error_remove_enabled() -> None:
    item = _make_item(status=AudiobookItemStatus.ERROR)
    widget = AudiobookQueueItemWidget(item)
    assert widget.remove_button.isEnabled()
...
def test_update_from_idempotent() -> None:
    item = _make_item()
    widget = AudiobookQueueItemWidget(item)
...
def test_processing_then_ready_reenables_remove() -> None:
    item = _make_item()
    widget = AudiobookQueueItemWidget(item)
