...
class TestComboPopulation:
    def test_shows_only_favorites_plus_sentinel(self):
    ...
    def test_unfavorited_active_theme_appears_at_top(self):
    ...
    def test_active_theme_is_selected(self):
...
class TestSentinelBehavior:
    def test_sentinel_emits_open_theme_settings(self):
        widget = HeaderWidget()
        captured: list[None] = []
    ...
    def test_sentinel_does_not_change_active_theme(self):
        widget = HeaderWidget()
...
class TestRefreshFavorites:
    def test_refresh_does_not_emit_theme_changed(self):
        widget = HeaderWidget()
        emitted: list[str] = []
    ...
    def test_refresh_preserves_selection_when_active_still_in_favorites(self):
        widget = HeaderWidget()
        Theme.set_mode("dark")
...
class TestThemeChangedSignal:
    def test_picking_a_real_theme_emits_theme_changed(self):
        Theme.set_favorites(("light", "dark"))
        widget = HeaderWidget()
        emitted: list[str] = []
        widget.theme_changed.connect(emitted.append)
