...
class TestDownloadToTemp:
    def test_success_writes_part_temp_and_returns_it(self, tmp_path):
        with patch.object(resource_downloader._session, "get", return_value=_response(content=b"abc123")):
    ...
    def test_creates_dest_dir_if_missing(self, tmp_path):
        ...
        with patch.object(resource_downloader._session, "get", return_value=_response()):
    ...
    def test_does_not_create_final_destination(self, tmp_path):
        ...
        with patch.object(resource_downloader._session, "get", return_value=_response(content=b"xyz")):
    ...
    def test_cancellation_before_request_raises_no_files(self, tmp_path):
        get = MagicMock()
        with (
            patch.object(resource_downloader._session, "get", get),
    ...
    def test_http_error_raises_setup_error_no_temp(self, tmp_path):
        with (
            patch.object(resource_downloader._session, "get", return_value=_response(status_code=404)),
    ...
    def test_network_failure_raises_setup_error_no_temp(self, tmp_path):
        with (
            patch.object(resource_downloader._session, "get", side_effect=requests.ConnectionError("boom")),
            pytest.raises(SetupError),
        ):
            download_to_temp(URL, dest_dir=tmp_path)
        assert _part_files(tmp_path) == []
