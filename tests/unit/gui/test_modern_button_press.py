...
def _middle(button) -> QColor:
    """The fill, sampled clear of the rounded corners and the text baseline."""
...
class TestPressFeedback:
    ...
    def test_a_quiet_button_tints_against_the_page_not_an_accent(self, qtbot, page_colour, variant):
        """Transparent variants show the page; a dark page needs a light tint.

        ``danger`` is one of them under D41: it stopped being a solid red fill
        and became a red outline over the page. Reading it as filled would keep
        the tint black, and black on a near-black page is nothing at all.
        """
        page_colour("#11111b")
    ...
    def test_a_filled_button_tints_against_its_own_fill(self, qtbot, page_colour, variant):
        """Its fill is opaque, so the page colour behind it is irrelevant."""
        page_colour("#11111b")
    ...
    @pytest.mark.parametrize("state", ["setChecked", "setDefault"])
    def test_a_quiet_button_that_state_has_filled_tints_against_the_fill(self, qtbot, page_colour, state):
        """The role is only half the answer: ``:checked`` and ``:default`` both
        hand a quiet variant the accent fill, and a fill is what the tint has to
        be measured against."""
        page_colour("#11111b")
        button = ModernButton("Folder", variant="secondary")
        qtbot.addWidget(button)
        button.setCheckable(True)
        getattr(button, state)(True)

        assert button.paints_its_own_fill()
        assert button.press_overlay_colour().lightness() == 0
    ...
    def test_the_stylesheets_static_pressed_swap_no_longer_reaches_this_class(self, qtbot, qapp):
        ...
        modern_at_rest, plain_at_rest = _middle(modern), _middle(plain)
        modern.setDown(True)
        plain.setDown(True)

        assert _middle(plain) != plain_at_rest, "the plain-button fallback lost its pressed state"
        assert _middle(modern) == modern_at_rest

    @pytest.mark.parametrize("variant", ["primary", "secondary", "ghost", "danger", "critical"])
    def test_no_role_swaps_its_fill_the_instant_it_is_pressed(self, qtbot, qapp, variant):
        """Read the pixels for every role, because D41 moved which ones swap.

        Under D41 the base ``QPushButton`` *is* the quiet control, so the base
        ``:pressed`` swap now reaches ``secondary`` and every unmarked button —
        it is no longer only ``primary`` that has to be neutralised.
        """
        qapp.setStyleSheet(Theme.get_stylesheet("light"))
        button = ModernButton("Mine Episode", variant=variant)
        qtbot.addWidget(button)
        button.resize(140, 36)

        at_rest = _middle(button)
        button.setDown(True)

        assert _middle(button) == at_rest

    def test_a_checked_toggle_keeps_its_accent_while_pressed(self, qtbot, qapp):
        """The neutralisation must not outrank ``:checked``.

        ``ModernButton:pressed`` and ``QPushButton:checked`` carry the same CSS
        specificity, so the one later in the sheet wins. Put the neutralisation
        after the checked block and a held toggle goes transparent — its state
        disappears for as long as the finger is down.
        """
        qapp.setStyleSheet(Theme.get_stylesheet("light"))
        toggle = ModernButton("Folder", variant="secondary")
