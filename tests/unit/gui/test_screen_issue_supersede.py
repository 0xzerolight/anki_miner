...
_MOKURO_AVAILABLE = "anki_miner.gui.widgets.mokuro_tab.MokuroTab._compute_mokuro_available"
...
@pytest.fixture
def mokuro_tab(qapp, qtbot, test_config):
    with patch(_MOKURO_AVAILABLE, return_value=True):
        tab = MokuroTab(test_config)
        qtbot.addWidget(tab)
        assert tab._availability_worker is not None
        assert tab._availability_worker.wait(3000)
