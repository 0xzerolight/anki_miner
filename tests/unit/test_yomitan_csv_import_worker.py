"""Tests for YomitanCsvImportWorker (merged pitch + frequency import worker)."""
...
class TestFrequencyImport:
    """The worker driving import_yomitan_freq_zip."""
    ...
    def test_import_emits_finished_with_result(self, tmp_path: Path, qapp):
        ...
        worker = YomitanCsvImportWorker(import_yomitan_freq_zip, zip_path, dest_csv)
    ...
    def test_progress_emitted_per_meta_bank(self, tmp_path: Path, qapp):
        ...
        worker = YomitanCsvImportWorker(import_yomitan_freq_zip, zip_path, tmp_path / "frequency.csv")
    ...
    def test_failed_emit_on_corrupt_zip(self, tmp_path: Path, qapp):
        bad = tmp_path / "bad.zip"
        bad.write_bytes(b"not a zip")
        worker = YomitanCsvImportWorker(import_yomitan_freq_zip, bad, tmp_path / "frequency.csv")
