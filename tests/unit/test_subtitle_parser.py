...
# ---------------------------------------------------------------------------
# Task 2.1 — Kana-only words via dictionary attestation
# ---------------------------------------------------------------------------


_KANA_ALLOWED_POS = frozenset({"名詞", "動詞", "形容詞", "副詞", "形状詞", "代名詞"})
_KANA_EXCLUDED_SUBTYPES = frozenset({"非自立", "数詞", "接尾", "助動詞", "接頭", "固有名詞"})


def _kana_rule():
    from anki_miner.services.morphology import TokenInclusionRule

    return TokenInclusionRule(allowed_pos=_KANA_ALLOWED_POS, excluded_subtypes=_KANA_EXCLUDED_SUBTYPES)


class _ProbeSpy:
    """Counting TermLookup: attests a fixed headword set, records every call.

    Wraps the same batched ``offline_terms_exist`` seam the real probe uses so a
    test can assert probe COUNT (one round-trip per line, never per-word) and
    count-vs-mine probe parity (T-38).
    """
    ...
    def __call__(self, terms):
        ...
        return self.dictionary & set(terms)


class TestKanaCandidateGate:
    """Tri-state gate: pure-hiragana content words become KANA_CANDIDATE."""

    def test_classify_pure_hiragana_content_word_is_candidate(self):
        from anki_miner.services.morphology import TokenDecision

        rule = _kana_rule()
        tok = _make_token("ごまかし", "動詞", "一般", lemma="ごまかす", orth_base="ごまかす")
        assert rule.classify(tok) is TokenDecision.KANA_CANDIDATE

    def test_classify_terminal_verdicts_unchanged(self):
        from anki_miner.services.morphology import TokenDecision

        rule = _kana_rule()
        assert rule.classify(_make_token("食べる", "動詞", lemma="食べる")) is TokenDecision.ACCEPT
        assert rule.classify(_make_token("勉強", "名詞", lemma="勉強")) is TokenDecision.ACCEPT
        assert rule.classify(_make_token("バス", "名詞", lemma="バス")) is TokenDecision.ACCEPT
        assert rule.classify(_make_token("あ", "名詞", lemma="あ")) is TokenDecision.REJECT  # <2 chars
        assert rule.classify(_make_token("を", "助詞", lemma="を")) is TokenDecision.REJECT  # particle

    def test_classify_mixed_kana_is_not_a_candidate(self):
        """Only a clean hiragana run qualifies — mixed hira+kata stays REJECT."""
        from anki_miner.services.morphology import TokenDecision

        rule = _kana_rule()
        assert rule.classify(_make_token("てすト", "名詞", lemma="てすト")) is TokenDecision.REJECT

    def test_should_include_candidate_needs_attested_lemma(self):
        rule = _kana_rule()
        tok = _make_token("ごまかし", "動詞", "一般", lemma="ごまかす", orth_base="ごまかす")
        assert rule.should_include(tok) is False  # None → reject (byte-identical default)
        assert rule.should_include(tok, frozenset()) is False  # empty → reject
        assert rule.should_include(tok, frozenset({"ごまかす"})) is True  # attested → include

    def test_flag_off_parser_rejects_kana_candidate(self, test_config):
        with patch("anki_miner.services.subtitle_parser.get_shared_tagger"):
            service = SubtitleParserService(test_config)  # flag off, no probe
        tok = _make_token("ごまかし", "動詞", "一般", lemma="ごまかす", orth_base="ごまかす")
        assert service._should_include_word(tok) is False
        assert service._kana_probe is None


def _kana_config(base, **over):
    from dataclasses import replace as dc_replace

    return dc_replace(base, mine_kana_only_words=True, **over)


def _kana_line_tokens():
    """Tokens for '彼はごまかしてうなずいた' — two kana candidates + a kanji word.

    彼 (kanji pronoun, always mined), ごまかし (lemma ごまかす — kana headword),
    うなずい (lemma 頷く — kanji headword, source kana orthBase うなずく).
    """
    return [
        _make_token("彼", "代名詞", "*", lemma="彼", kana="カレ", orth_base="彼"),
        _make_token("は", "助詞", "係助詞", lemma="は", kana="ハ"),
        _make_token("ごまかし", "動詞", "一般", lemma="ごまかす", kana="ゴマカシ", orth_base="ごまかす"),
        _make_token("て", "助詞", "接続助詞", lemma="て", kana="テ"),
        _make_token("うなずい", "動詞", "一般", lemma="頷く", kana="ウナズイ", orth_base="うなずく"),
        _make_token("た", "助動詞", "*", lemma="た", kana="タ"),
    ]


_KANA_LINE_TEXT = "彼はごまかしてうなずいた"


class TestKanaOnlyMiningIntegration:
    """Mock-tagger coverage of the kana-attestation seam in _iter_parsed_lines."""

    def _run(self, tmp_path, config, text, tokens, dictionary=None, *, method="parse", term_lookup=None):
        sub_file = tmp_path / "test.srt"
        sub_file.write_text("stub", encoding="utf-8")
        mock_line = MagicMock()
        mock_line.text = text
        ...
        mock_tagger = MagicMock()
        mock_tagger.return_value = tokens
        lookup = term_lookup if term_lookup is not None else _lookup_for(dictionary or set())
        with (
            patch("anki_miner.services.subtitle_parser.pysubs2.load", return_value=mock_subs),
            patch("anki_miner.services.subtitle_parser.get_shared_tagger", return_value=mock_tagger),
        ):
            service = SubtitleParserService(config, term_lookup=lookup)
            if method == "count":
                return service.count_lemmas(sub_file)
            return service.parse_subtitle_file(sub_file)

    def test_attested_kana_word_is_mined_when_flag_on(self, tmp_path, test_config):
        words = self._run(
            tmp_path, _kana_config(test_config), _KANA_LINE_TEXT, _kana_line_tokens(), {"ごまかす", "頷く"}
        )
        assert {w.lemma for w in words} == {"彼", "ごまかす", "頷く"}

    def test_kana_word_dropped_when_flag_off(self, tmp_path, test_config):
        # Same tokens + a lookup that WOULD attest, but the flag is off → dropped.
        words = self._run(tmp_path, test_config, _KANA_LINE_TEXT, _kana_line_tokens(), {"ごまかす", "頷く"})
        assert {w.lemma for w in words} == {"彼"}

    def test_unattested_kana_word_stays_dropped_with_flag_on(self, tmp_path, test_config):
        # Flag on but the dictionary attests neither candidate.
        words = self._run(tmp_path, _kana_config(test_config), _KANA_LINE_TEXT, _kana_line_tokens(), set())
        assert {w.lemma for w in words} == {"彼"}

    def test_orthbase_only_attestation(self, tmp_path, test_config):
        """A candidate whose lemma is kanji attests via its source-kana orthBase."""
        tokens = [
            _make_token("しゃべっ", "動詞", "一般", lemma="喋る", kana="シャベッ", orth_base="しゃべる"),
            _make_token("た", "助動詞", "*", lemma="た", kana="タ"),
        ]
        # Dictionary stores only the kana orthBase, NOT the kanji lemma.
        words = self._run(tmp_path, _kana_config(test_config), "しゃべった", tokens, {"しゃべる"})
        assert {w.lemma for w in words} == {"喋る"}
        # Verb mines as source-orthography orthBase.
        assert words[0].mined_form == "しゃべる"

    def test_degradation_no_probe_byte_identical(self, tmp_path, test_config):
        """Flag on but NO probe injected (no offline dict) == flag off output."""
        on_no_probe = self._run(
            tmp_path, _kana_config(test_config), _KANA_LINE_TEXT, _kana_line_tokens(), method="parse", term_lookup=None
        )
        off = self._run(tmp_path, test_config, _KANA_LINE_TEXT, _kana_line_tokens(), {"ごまかす", "頷く"})
        assert {w.lemma for w in on_no_probe} == {w.lemma for w in off} == {"彼"}

    def test_t38_count_and_parse_agree_flag_on(self, tmp_path, test_config):
        """T-38 canary: count_lemmas keys == mined lemma set with the flag on."""
        config = _kana_config(test_config)
        words = self._run(tmp_path, config, _KANA_LINE_TEXT, _kana_line_tokens(), {"ごまかす", "頷く"})
        counts = self._run(tmp_path, config, _KANA_LINE_TEXT, _kana_line_tokens(), {"ごまかす", "頷く"}, method="count")
        assert {w.lemma for w in words} == set(counts.keys()) == {"彼", "ごまかす", "頷く"}

    def test_probe_parity_and_batching(self, tmp_path, test_config):
        """count_lemmas and mining each issue one batched probe over the file.

        Fresh parsers (no shared line cache): both make exactly one probe call
        for the single line, and that call carries every candidate's terms at
        once (batched, never per-word) — the T-38 probe-parity guarantee.
        Compound matching is off so the spy sees ONLY the kana probe.
        """
        config = _kana_config(test_config, compound_matching=False)
        spy_count = _ProbeSpy({"ごまかす", "頷く"})
        self._run(tmp_path, config, _KANA_LINE_TEXT, _kana_line_tokens(), method="count", term_lookup=spy_count)
        spy_mine = _ProbeSpy({"ごまかす", "頷く"})
        self._run(tmp_path, config, _KANA_LINE_TEXT, _kana_line_tokens(), method="parse", term_lookup=spy_mine)

        assert len(spy_count.calls) == len(spy_mine.calls) == 1  # one line → one probe each
        # Two candidates resolved in a single batched call, not two per-word calls.
        assert len(spy_mine.calls[0]) >= 2

    def test_same_instance_cache_reuses_baked_probe(self, tmp_path, test_config):
        """Deck Builder double-parse: count then parse on ONE instance probes once.

        The attested set is baked into the per-file line cache, so the second
        pass replays it without re-issuing the probe. Compound matching is off
        so the spy sees ONLY the kana probe.
        """
        config = _kana_config(test_config, compound_matching=False)
        spy = _ProbeSpy({"ごまかす", "頷く"})
        sub_file = tmp_path / "test.srt"
        sub_file.write_text("stub", encoding="utf-8")
        import os
        ...
        os.utime(sub_file, (1000, 1000))
        mock_line = MagicMock()
        mock_line.text = _KANA_LINE_TEXT
        mock_line.start = 1000
        mock_line.end = 3000
        mock_subs = MagicMock()
        mock_subs.__iter__ = MagicMock(side_effect=lambda: iter([mock_line]))
        mock_tagger = MagicMock()
        mock_tagger.return_value = _kana_line_tokens()
        with (
            patch("anki_miner.services.subtitle_parser.pysubs2.load", return_value=mock_subs),
            patch("anki_miner.services.subtitle_parser.get_shared_tagger", return_value=mock_tagger),
        ):
            service = SubtitleParserService(config, term_lookup=spy)
            counts = service.count_lemmas(sub_file)
            words = service.parse_subtitle_file(sub_file)

        assert len(spy.calls) == 1  # probe issued once; second pass hit the cache
        assert {w.lemma for w in words} == set(counts.keys()) == {"彼", "ごまかす", "頷く"}

    def test_index_path_carries_kana_candidate(self, tmp_path, test_config):
        """parse_subtitle_file_with_index sees the same attested kana lemmas."""
        config = _kana_config(test_config)
        sub_file = tmp_path / "test.srt"
        sub_file.write_text("stub", encoding="utf-8")
        mock_line = MagicMock()
        ...
        with (
            ...
            service = SubtitleParserService(config, term_lookup=_lookup_for({"ごまかす", "頷く"}))
            words, lines = service.parse_subtitle_file_with_index(sub_file)

        assert {w.lemma for w in words} == {"彼", "ごまかす", "頷く"}
        assert lines[0].lemmas == {"彼", "ごまかす", "頷く"}


@pytest.mark.skipif(not _fugashi_available(), reason="fugashi/unidic-lite not installed")
class TestKanaOnlyRealFugashi:
    """End-to-end kana attestation over real unidic tokenization."""

    def _mine(self, tmp_path, line, dictionary, *, flag=True, name="kana.srt"):
        from dataclasses import replace as dc_replace

        srt_file = _write_srt(tmp_path, name, line)
        config = dc_replace(AnkiMinerConfig(media_temp_folder=tmp_path / "media"), mine_kana_only_words=flag)
        service = SubtitleParserService(config, term_lookup=_lookup_for(dictionary))
        return service.parse_subtitle_file(srt_file)

    def test_gomakasu_mined_when_attested(self, tmp_path):
        words = self._mine(tmp_path, "彼はうまくごまかした。", {"ごまかす"})
        assert "ごまかす" in {w.lemma for w in words}

    def test_unazuku_mined_via_kanji_lemma(self, tmp_path):
        # surface うなずいた is pure kana; unidic lemma is 頷く → attests via lemma.
        words = self._mine(tmp_path, "彼は黙ってうなずいた。", {"頷く"})
        assert "頷く" in {w.lemma for w in words}

    def test_flag_off_drops_kana_but_keeps_the_rest(self, tmp_path):
        line = "彼はうまくごまかした。"
        on = {w.lemma for w in self._mine(tmp_path, line, {"ごまかす"}, flag=True)}
        off = {w.lemma for w in self._mine(tmp_path, line, {"ごまかす"}, flag=False, name="off.srt")}
        assert "ごまかす" in on
        assert "ごまかす" not in off
        assert off.issubset(on)  # the flag only ADDS the attested kana word

    def test_unattested_kana_not_mined(self, tmp_path):
        words = self._mine(tmp_path, "彼はうまくごまかした。", set())
        assert "ごまかす" not in {w.lemma for w in words}

    def test_t38_count_matches_mine_flag_on(self, tmp_path):
        from dataclasses import replace as dc_replace

        srt_file = _write_srt(tmp_path, "kana.srt", "彼がごまかした。")
        config = dc_replace(AnkiMinerConfig(media_temp_folder=tmp_path / "media"), mine_kana_only_words=True)
        counts = SubtitleParserService(config, term_lookup=_lookup_for({"ごまかす"})).count_lemmas(srt_file)
        words = SubtitleParserService(config, term_lookup=_lookup_for({"ごまかす"})).parse_subtitle_file(srt_file)
        assert "ごまかす" in counts
        assert set(counts.keys()) == {w.lemma for w in words}

    def test_no_offline_dict_degrades_to_identical(self, tmp_path):
        from dataclasses import replace as dc_replace

        srt_file = _write_srt(tmp_path, "kana.srt", "彼はうまくごまかした。")
        base = AnkiMinerConfig(media_temp_folder=tmp_path / "media")
        # Flag ON but NO probe injected (no enabled offline dict) == flag off.
        on_no_probe = SubtitleParserService(dc_replace(base, mine_kana_only_words=True))
        off = SubtitleParserService(base)
        assert on_no_probe.parse_subtitle_file(srt_file) == off.parse_subtitle_file(srt_file)
