...
Single/Batch/YouTube/Deck Builder sub-tabs with per-child presenters.
"""
...
def test_video_tab_nests_four_inner_tabs(wired_window):
    """The container holds exactly four inner tabs: Single / Batch / YouTube / Deck Builder."""
    _window, _titles, tabs = wired_window
    video = tabs["Video"]
    assert video._inner_tabs.count() == 4
    labels = [video._inner_tabs.tabText(i) for i in range(video._inner_tabs.count())]
    assert labels == ["Single", "Batch", "YouTube", "Deck Builder"]


def test_video_tab_inner_child_types(wired_window):
    """Inner tabs are the Single, Batch, YouTube, and Deck Builder sub-tabs, in order."""
    _window, _titles, tabs = wired_window
    video = tabs["Video"]
    ...
    assert isinstance(video._inner_tabs.widget(3), DeckBuilderTab)
    ...
    assert video.deck_builder_tab is video._inner_tabs.widget(3)


def test_video_tab_children_have_distinct_presenters(wired_window):
    """Per-child presenters: Single/Batch/Deck Builder wire presenter signals
    into their own log widgets, so sharing one would cross-post between
    sub-tabs."""
    _window, _titles, tabs = wired_window
    video = tabs["Video"]
    presenters = (
        ...
        video.deck_builder_tab.presenter,
    )
