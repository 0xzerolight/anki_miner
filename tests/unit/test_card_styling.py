...
class TestBuildManagedBlock:
    def test_includes_both_markers(self):
        block = build_managed_block(use_default=True, custom_css="")
    ...
    def test_default_omitted_when_disabled(self):
        block = build_managed_block(use_default=False, custom_css="")
        # No default body — just the two markers back to back.
        assert ".yomitan-glossary" not in block
        assert block == f"{BEGIN_MARKER}\n{END_MARKER}"

    def test_custom_css_appended(self):
        block = build_managed_block(use_default=False, custom_css=".x{color:red}")
...
class TestApplyManagedBlock:
    ...
    def test_into_empty_css(self):
        block = build_managed_block(use_default=False, custom_css=".x{}")
    ...
    def test_idempotent_no_duplicate_block(self):
        block = build_managed_block(use_default=True, custom_css=".x{}")
    ...
    def test_replace_preserves_surrounding_user_css(self):
        block_v1 = build_managed_block(use_default=False, custom_css=".v1{}")
        block_v2 = build_managed_block(use_default=False, custom_css=".v2{}")
...
class TestStripManagedBlock:
    ...
    def test_apply_then_strip_round_trips_to_original(self):
        block = build_managed_block(use_default=True, custom_css=".x{color:red}")
        original = ".card{font-size:20px}"
