...
def test_excluded_decks_get_set_roundtrip():
...
def test_set_excluded_decks_replaces_previous():
    panel = FilteringSettingsPanel()
...
def test_default_excluded_decks_empty():
    panel = FilteringSettingsPanel()
    assert panel.get_excluded_decks() == ()


def test_remove_deletes_selected_row():
    panel = FilteringSettingsPanel()
...
def test_add_deck_emits_fetch_when_no_cache(qtbot=None):
...
def test_set_available_decks_caches_and_skips_already_excluded(monkeypatch):
