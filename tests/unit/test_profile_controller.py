...
class TestVideoPreviewSurvivesASwitch:
    """The video surface is a property of the HOST, not of a settings profile.

    Building it on a machine whose GL driver cannot load aborts the process, so
    a profile written on a working machine must not be able to switch it back
    on here — that would silently undo the crash-marker rescue and kill the app
    on the next video mine.
    """

    def test_a_profile_with_it_on_cannot_re_enable_it(self, controller, window, profile_b):
        _seed("b", replace(profile_b, video_preview_enabled=True), "B")
        window.config = replace(window.config, video_preview_enabled=False)
    ...
    def test_the_rest_of_the_profile_still_applies(self, controller, window, profile_b):
        """Held back, not ignored: everything else about the switch is normal."""
        _seed("b", replace(profile_b, video_preview_enabled=True), "B")
        window.config = replace(window.config, video_preview_enabled=False)

        controller.switch_to("b")

        assert window.config.anki_deck_name == profile_b.anki_deck_name

    def test_a_working_host_keeps_its_preview_on(self, controller, window, profile_b):
        """The hold-back is symmetric: a profile that has it off must not turn
        it off for someone whose machine is fine."""
        _seed("b", replace(profile_b, video_preview_enabled=False), "B")
        window.config = replace(window.config, video_preview_enabled=True)
